...
// Changed namespace to prevent conflict with original System.Xml.Linq
namespace SkylineToolkit.Xml.XPath
{
    internal class XNodeNavigator : XPathNavigator, IXmlLineInfo
    {
        ...
        {
            throw new NotSupportedException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.NotSupported_MoveToId));
        }
        ...
        {
            XContainer c = source as XContainer;
            if (c == null) throw new InvalidOperationException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.InvalidOperation_BadNodeType, NodeType));
    ...
    struct XPathEvaluator
    {
        public object Evaluate<T>(XNode node, string expression, IXmlNamespaceResolver resolver) where T : class
        {
            ...
            if (!(result is T)) throw new InvalidOperationException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.InvalidOperation_UnexpectedEvaluation, result.GetType()));
            return (T)result;
        }
        ...
        IEnumerable<T> EvaluateIterator<T>(XPathNodeIterator result)
        {
            foreach (XPathNavigator navigator in result)
            {
                ...
                if (!(r is T)) throw new InvalidOperationException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.InvalidOperation_UnexpectedEvaluation, r.GetType()));
                yield return (T)r;
                XText t = r as XText;
    ...
    public static class Extensions
    {
        ...
        public static XPathNavigator CreateNavigator(this XNode node, XmlNameTable nameTable)
        {
            ...
            if (node is XDocumentType) throw new ArgumentException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.Argument_CreateNavigator, XmlNodeType.DocumentType));
            XText text = node as XText;
            if (text != null)
            {
                if (text.parent is XDocument) throw new ArgumentException(SkylineToolkit.Xml.Linq.Res.GetString(SkylineToolkit.Xml.Linq.Res.Argument_CreateNavigator, XmlNodeType.Whitespace));
